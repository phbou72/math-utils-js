...
(function() {
  MathUtils.EquationParser = (function() {
    ...
    EquationParser.prototype.events = {};
    ...
    EquationParser.prototype.formatNumber = function(number) {
      if (number.length === 0) {
        return "";
      }
      if (number.indexOf("e") !== -1) {
        return this.truncateExponential(number);
      }
      if (number.length > 10) {
        if (number.indexOf(".") === -1) {
          return this.truncateInteger(number);
        } else {
          return this.truncateFloat(number);
        }
      } else {
        return number;
      }
    ...
    EquationParser.prototype.truncateInteger = function(str) {
      var firstDigit, secondPart;
    ...
    EquationParser.prototype.truncateFloat = function(str) {
      var dotPos, firstPart, nbDigitsToKeep;

      dotPos = str.indexOf(".");
      firstPart = str.substring(0, dotPos);
      if (firstPart.length >= 10) {
        return this.truncateInteger(str);
      } else {
    ...
    EquationParser.prototype.truncateExponential = function(str) {
      var ePos, eSign, firstPart;
...
(function() {
  ...
  MathUtils.GraphPaper = (function() {
    ...
    function GraphPaper(selector, width, height) {
      ...
      this.getAddedPaths = __bind(this.getAddedPaths, this);
    ...
    GraphPaper.prototype.initToolsButton = function() {
      ...
      return _.each(tools, function(tool) {
        ...
        return d3.select(button).on("click", function(element, id) {
          return _this.switchToTool(tool);
        });
      });
    };
    ...
    GraphPaper.prototype.showAddVertexHandle = function() {
      ...
      if (currentTool === "move" && !draggingVertex && !draggingPath) {
        if ((_ref = d3.event.srcElement.nodeName) === "path" || _ref === "circle") {
    ...
    GraphPaper.prototype.deletePath = function() {
      ...
      if (d3.event.toElement.tagName === "path") {
        path = d3.event.toElement;
        parentPath = d3.select(path.parentNode)[0][0].parentElement;
        pathId = this.findParentPathIndex(parentPath);
        if (!this.isShapeEditable(parentPath)) {
    ...
    GraphPaper.prototype.getAddedPaths = function() {
      var addedPath, i, line, linesVertexsCopy, vertex, _i, _len, _results;

      linesVertexsCopy = [];
      addedPath = linesTracer.selectAll("g.added-line");
      _results = [];
      for (i = _i = 0, _len = linesVertexs.length; _i < _len; i = ++_i) {
        line = linesVertexs[i];
        if (line.length !== 0) {
          linesVertexsCopy.push([]);
          _results.push((function() {
            ...
            for (_j = 0, _len1 = line.length; _j < _len1; _j++) {
              vertex = line[_j];
              _results1.push(linesVertexsCopy[i].push(this.posToRealPos(vertex)));
            }
            return _results1;
          }).call(this));
        } else {
