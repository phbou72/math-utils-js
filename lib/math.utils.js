...
MathUtils.GraphPaper = (function() {
  var axis, axisNumbers, currentDrawMode, currentTool, domainX, doubleClickTimer, draggedPath, draggedPathId, draggedVertex, draggedVertexId, draggingPath, draggingVertex, drawModes, grid, gridSize, height, hideAddVertexHandleTimer, initialPathEditable, isDoubleClick, lastLine, lastLineIndex, lastMousePosition, linePointer, linesTracer, linesVertexs, node, origin, polygonPointer, registeredEvent, space, svg, tools, vertexCoordinates, vertexPointer, vertexSize, width;
  ...
  GraphPaper.prototype.showCurrentTool = function() {
    switch (currentTool) {
      ...
      case "function":
        return this.showFunctionPlotModal();
      default:
        return null;
    }
  };
  ...
  GraphPaper.prototype.addNumbersToAxis = function() {
    var coords1, coords2, i, nbLines, numbersCoord, pos, x1, x2, y1, y2, zeroOriginCoords, _i;
    ...
    for (i = _i = 0; 0 <= nbLines ? _i <= nbLines : _i >= nbLines; i = 0 <= nbLines ? ++_i : --_i) {
      ...
      coords1.number = this.posToRealPos(coords1).x;
      ...
      coords2.number = this.posToRealPos(coords2).y;
      numbersCoord.push(coords2);
    }
    numbersCoord = numbersCoord.filter(function(coords) {
      return coords.number !== 0;
    });
    zeroOriginCoords = {
...
w({
  drawVertexs: function(path, vertexs) {
    ...
    vertexsCircle.enter().append("circle").attr("class", "path-vertex").attr("r", vertexSize).on("mousedown", _this.onMouseDownVertex).on("mouseup", _this.onMouseUpVertex);
...
({
  drawPath: function(path, vertexs, shapeType) {
    var lineFunction, linesType, pathGroup;

    linesType = {
      polygon: "linear-closed",
      line: "linear",
      "function": "monotone"
    };
    ...
    if (shapeType !== "function") {
      path.on("mousemove", _this.showAddVertexHandle).on("mouseout", _this.hideVertexHandle);
    }
    path.on("mousedown", _this.onMouseDownPath).on("mouseup", _this.onMouseUpPath);
  ...
  onMouseDownPath: function(data, pathId) {
    switch (currentTool) {
      case "move":
        return _this.startDraggingPath();
    }
  },
  onMouseUpPath: function(data, pathId) {
    switch (currentTool) {
      case "move":
        return _this.stopDraggingPath();
      case "delete":
        return _this.deletePath();
    }
  },
  onMouseMovePath: function() {
    if (currentTool === "move") {
      return _this.updatePathPositions();
    }
  },
  onMouseDownVertex: function(data, vertexId) {
    switch (currentTool) {
      case "move":
        return _this.startDraggingVertex(vertexId);
    }
  },
  onMouseUpVertex: function(data, vertexId) {
    switch (currentTool) {
      case "move":
        return _this.stopDraggingVertex();
      case "delete":
        return _this.deleteVertex(vertexId);
    }
  },
  onMouseMoveVertex: function() {
    switch (currentTool) {
      case "move":
        return _this.updateVertexPosition();
    }
  },
  addVertex: function() {
    var coords, lastLine;
    ...
    coords = {
      x: parseFloat(vertexPointer.attr("cx")),
      y: parseFloat(vertexPointer.attr("cy"))
    };
    ...
    _this.drawVertexs(lastLine, linesVertexs[lastLineIndex]);
    linesVertexs.push([]);
    lastLineIndex++;
    return _this.switchToTool("move");
  },
  showVertexTool: function() {
    var coord;

    d3.event.preventDefault();
    coord = _this.getDrawingCoordinates(_this.getEventCoordinates());
    _this.showVertexPointer(coord);
    return _this.showVertexCoordinates(coord);
  },
  addLine: function() {
    var coords, doubleClickTimer, isDoubleClick, lastLine;

    d3.event.preventDefault();
    if (isDoubleClick === false) {
      isDoubleClick = true;
      doubleClickTimer = setTimeout(_this.timerEndDoubleClick, 200);
      coords = {
        x: parseFloat(vertexPointer.attr("cx")),
        y: parseFloat(vertexPointer.attr("cy"))
      };
      ...
      _this.drawPath(lastLine, linesVertexs[lastLineIndex], "line");
      return _this.drawVertexs(lastLine, linesVertexs[lastLineIndex]);
    } else {
  ...
  timerEndDoubleClick: function() {
    var isDoubleClick;

    return isDoubleClick = false;
  },
  showLineTool: function() {
    var coord;

    d3.event.preventDefault();
    coord = _this.getDrawingCoordinates(_this.getEventCoordinates());
    _this.showVertexPointer(coord);
    _this.showVertexCoordinates(coord);
    if (linesVertexs[lastLineIndex].length > 0) {
      return _this.showLinePointer(coord);
    }
  },
  addPolygon: function() {
    var coords, doubleClickTimer, isDoubleClick, lastLine;
    ...
    if (isDoubleClick === false) {
      ...
      _this.drawPath(lastLine, linesVertexs[lastLineIndex], "polygon");
      return _this.drawVertexs(lastLine, linesVertexs[lastLineIndex]);
    } else {
      linesTracer.select(".line-pointer").attr("style", "display:none");
      linesVertexs.push([]);
  ...
  showPolygonTool: function() {
    ...
    if (linesVertexs[lastLineIndex].length >= 2) {
      return _this.showPolygonPointer(coord);
    } else {
      return _this.showLinePointer(coord);
    }
  },
  addFunctionPlot: function() {
    var expression, group, segment, segments, variable, _i, _len;

    variable = $("#inputVariable").val();
    expression = $("#inputExpression").val();
    segments = _this.interpolateFunction(variable, expression);
    group = linesTracer.append("g").attr("class", "path added-function");
    for (_i = 0, _len = segments.length; _i < _len; _i++) {
      segment = segments[_i];
      _this.drawPath(group, segment, "function");
    }
    linesVertexs.push(segments);
    lastLineIndex++;
    return $("#functionPlotModal").modal("hide");
  },
  showFunctionPlotModal: function() {
    var modal;

    modal = $("#functionPlotModal").modal({});
    modal.on("hide", _this.hideFunctionPlotModal);
    return $("#addFunctionButton").on("click", _this.addFunctionPlot);
  },
  hideFunctionPlotModal: function() {
    var expression, variable;

    _this.switchToTool("move");
    variable = $("#inputVariable").val("x");
    return expression = $("#inputExpression").val("");
  },
  validateFunctionExpression: function(variable, expression) {},
  interpolateFunction: function(variable, expression) {
    var actualSegment, exp, parser, segments, x, y, _i, _ref, _ref1;

    parser = new MathUtils.EquationParser();
    segments = [];
    actualSegment = [];
    for (x = _i = _ref = domainX[0], _ref1 = domainX[1]; 0.01 > 0 ? _i <= _ref1 : _i >= _ref1; x = _i += 0.01) {
      exp = expression.replace(variable, x);
      y = parser.evaluate(exp);
      if (!_.isFinite(y)) {
        if (!_.isEmpty(actualSegment)) {
          segments.push(actualSegment);
          actualSegment = [];
        }
      } else {
        actualSegment.push(_this.realPosToPos({
          x: x,
          y: y
        }));
      }
  ...
  showAddVertexHandle: function() {
    ...
    if (currentTool === "move" && !draggingVertex && !draggingPath) {
      if ((_ref = d3.event.target.nodeName) === "path" || _ref === "circle") {
        ...
        pathIndex = _this.findParentPathIndex(parentPath);
      }
      if (!_this.isShapeEditable(parentPath)) {
        return;
      }
      ...
      shapeType = _this.getParentPathShapeType(parentPath);
      handlesCoord = _this.getHandlesCoord(shapeType, linesVertexs[pathIndex]);
      return _this.updateHandles(parentPath, shapeType, handlesCoord);
    } else {
      return _this.removeVertexHandles();
    }
  },
  updateHandles: function(parentPath, shapeType, handlesCoord) {
    var handles;

    handles = d3.select(parentPath).select("g.handles").selectAll("circle.handle").data(handlesCoord);
    handles.enter().append("circle").attr("class", "handle").attr("r", vertexSize).on("click", _this.onHandleClickAddVertex);
  ...
  hideVertexHandle: function() {
    var hideAddVertexHandleTimer;

    return hideAddVertexHandleTimer = setTimeout(_this.removeVertexHandles, 150);
  },
  removeVertexHandles: function() {
    return linesTracer.selectAll("circle.handle").remove();
  },
  getHandlesCoord: function(shapeType, vertexs) {
  ...
  onHandleClickAddVertex: function(data, handleId) {
    var coords, handle, parentPath, pathIndex, shapeType;

    handle = d3.select(d3.event.target);
    parentPath = d3.select(handle.parentNode)[0][0].parentNode;
    pathIndex = _this.findParentPathIndex(parentPath);
    coords = {
      x: parseFloat(handle.attr("cx")),
      y: parseFloat(handle.attr("cy"))
    };
    linesVertexs[pathIndex].splice(handleId + 1, 0, coords);
    shapeType = _this.getParentPathShapeType(parentPath);
    _this.drawPath(d3.select(parentPath), linesVertexs[pathIndex], shapeType);
    return _this.drawVertexs(d3.select(parentPath), linesVertexs[pathIndex]);
  },
  startDraggingVertex: function(vertexId) {
    var draggedVertex, draggedVertexId, draggingVertex;

    draggingVertex = true;
    draggedVertex = d3.event.target;
    draggedVertexId = vertexId;
    _this.registerSVGEvent("mousemove", _this.onMouseMoveVertex);
    return _this.registerSVGEvent("mouseup", _this.onMouseUpVertex);
  },
  stopDraggingVertex: function() {
    var draggedVertex, draggedVertexId, draggingVertex;

    draggingVertex = false;
    draggedVertex = null;
    draggedVertexId = -1;
    _this.unregisterSVGEvent("mousemove", _this.onMouseMoveVertex);
    _this.unregisterSVGEvent("mouseup", _this.onMouseUpVertex);
    return _this.hideVertexCoordinates();
  },
  updateVertexPosition: function() {
    ...
    if (draggingVertex === true) {
      ...
      parentPathIndex = _this.findParentPathIndex(parentPath);
      if (!_this.isShapeEditable(parentPath)) {
        return;
      }
      ...
      if (pathNode.classed("added-polygon") || pathNode.classed("initial-polygon")) {
        return _this.drawPath(d3.select(parentPath), linesVertexs[parentPathIndex], "polygon");
      } else if (pathNode.classed("added-line") || pathNode.classed("initial-shape")) {
        return _this.drawPath(d3.select(parentPath), linesVertexs[parentPathIndex], "line");
      }
    }
  },
  startDraggingPath: function() {
    var draggedPath, draggedPathId, draggingPath, lastMousePosition, parentPath, parentPathId, path;

    path = d3.event.target;
    if (path.tagName === "path") {
      parentPath = path.parentNode.parentNode;
      parentPathId = _this.findParentPathIndex(parentPath);
      if (!_this.isShapeEditable(parentPath)) {
        return;
      }
      draggingPath = true;
      ...
      lastMousePosition = _this.getDrawingCoordinates(_this.getEventCoordinates());
      _this.registerSVGEvent("mousemove", _this.onMouseMovePath);
      return _this.registerSVGEvent("mouseup", _this.onMouseUpPath);
    }
  },
  stopDraggingPath: function() {
    var draggedPath, draggedPathId, draggingPath, lastMousePosition;

    draggingPath = false;
    draggedPath = null;
    draggedPathId = -1;
    lastMousePosition = _this.getDrawingCoordinates(_this.getEventCoordinates());
    _this.unregisterSVGEvent("mousemove", _this.onMouseMovePath);
    return _this.unregisterSVGEvent("mouseup", _this.onMouseUpPath);
  },
  updatePathPositions: function() {
    var actualMouseCoord, id, lastMousePosition, shapeType, translation, vertex, x, y, _i, _len, _ref;

    actualMouseCoord = _this.getDrawingCoordinates(_this.getEventCoordinates());
    if (typeof lastMousePosition !== "undefined" && lastMousePosition !== null) {
      ...
      shapeType = _this.getParentPathShapeType(draggedPath);
      _this.drawPath(d3.select(draggedPath), linesVertexs[draggedPathId], shapeType);
      return _this.drawVertexs(d3.select(draggedPath), linesVertexs[draggedPathId]);
    }
  },
  deleteVertex: function(vertexId) {
    var parentPath, pathId, pathNode, shapeType, vertex;

    vertex = d3.event.target;
    parentPath = d3.select(vertex.parentNode)[0][0].parentNode;
    pathNode = d3.select(parentPath);
    pathId = _this.findParentPathIndex(parentPath);
    if (!_this.isShapeEditable(parentPath)) {
      return;
    }
    ...
    if (linesVertexs[pathId].length === 0) {
      ...
      pathNode.remove();
      lastLineIndex--;
      return _this.drawVertexs(d3.select(parentPath), []);
    } else {
      shapeType = _this.getParentPathShapeType(pathParent);
      _this.drawVertexs(d3.select(parentPath), linesVertexs[pathId]);
      return _this.drawPath(pathNode, linesVertexs[pathId], shapeType);
    }
  },
  deletePath: function() {
  ...
  getDrawingCoordinates: function(coord) {
    switch (currentDrawMode) {
      case "free":
        return coord;
      case "snapToGrid":
        return _this.snapToGrid(coord);
      case "snapToIntersection":
        return _this.snapToIntersection(coord);
    }
  },
  snapToGrid: function(coord) {
    ...
    factor = 0.15;
    realCoord = _this.posToRealPos(coord);
  ...
  snapToIntersection: function(coord) {
  ...
  showVertexPointer: function(coord) {
    var vertexPointer;

    if (typeof vertexPointer === "undefined" || vertexPointer === null) {
  ...
  hideVertexPointer: function() {
    if (typeof vertexPointer !== "undefined" && vertexPointer !== null) {
      return vertexPointer.attr("style", "display: none");
    }
  },
  showVertexCoordinates: function(coord) {
    var realCoord, vertexCoordinates;

    if (typeof vertexCoordinates === "undefined" || vertexCoordinates === null) {
  ...
  hideVertexCoordinates: function() {
    if (typeof vertexCoordinates !== "undefined" && vertexCoordinates !== null) {
      return vertexCoordinates.attr("style", "display: none");
    }
  },
  showLinePointer: function() {
    var cx, cy, lastVertex, linePointer;

    if (typeof linePointer === "undefined" || linePointer === null) {
  ...
  findParentPathIndex: function(pathToFind) {
  ...
  getParentPathShapeType: function(parentPath) {
  ...
  posToRealPos: function(coord) {
    var realX, realY;

    realX = _this.truncateFloat((coord.x - origin.x) / gridSize);
    realY = _this.truncateFloat((origin.y - coord.y) / gridSize);
    return {
      x: realX,
      y: realY
    };
  },
  realPosToPos: function(coord) {
    return {
      x: _this.realXPosToPos(coord.x),
      y: _this.realYPosToPos(coord.y)
    };
  },
  realXPosToPos: function(pos) {
    return pos * gridSize + origin.x;
  },
  realYPosToPos: function(pos) {
    return origin.y - pos * gridSize;
  },
  truncateFloat: function(number) {
  ...
  getAddedShapes: function() {
    ...
    for (_i = 0, _len = paths.length; _i < _len; _i++) {
      path = paths[_i];
      pathId = _this.findParentPathIndex(path);
      _ref = ["vertex", "line", "polygon"];
      for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
        shapeType = _ref[_j];
  ...
  addInitialVertex: function(vertexs) {
    ...
    for (_i = 0, _len = vertexs.length; _i < _len; _i++) {
      ...
      linesVertexs[lastLineIndex].push(_this.realPosToPos(vertex));
      lastVertex = linesTracer.append("g").attr("class", "path added-vertex initial-shape");
      _this.drawVertexs(lastVertex, [_this.realPosToPos(vertex)]);
      linesVertexs.push([]);
      _results.push(lastLineIndex++);
    }
    return _results;
  },
  addInitialPath: function(paths, shapeType) {
    ...
    for (_i = 0, _len = paths.length; _i < _len; _i++) {
      ...
      for (_j = 0, _len1 = path.length; _j < _len1; _j++) {
        vertex = path[_j];
        vertexs.push(_this.realPosToPos(vertex));
      }
      linesVertexs[lastLineIndex] = vertexs;
      _this.drawInitialPath(vertexs, shapeType);
      linesVertexs.push([]);
      _results.push(lastLineIndex++);
    }
    return _results;
  },
  drawInitialPath: function(vertexs, shapeType) {
    var lastLine;

    lastLine = linesTracer.append("g").attr("class", "path added-" + shapeType + " initial-shape");
    _this.drawPath(lastLine, vertexs, shapeType);
    return _this.drawVertexs(lastLine, vertexs);
  },
  addShapes: function(shapes) {
    if (shapes.vertexs != null) {
      _this.addInitialVertex(shapes.vertexs);
    }
    if (shapes.lines != null) {
      _this.addInitialPath(shapes.lines, "line");
    }
    if (shapes.polygons != null) {
      return _this.addInitialPath(shapes.polygons, "polygon");
    }
  },
  registerSVGEvent: function(eventName, callback) {
    if (registeredEvent[eventName] == null) {
      svg.on(eventName, _this.fireCallback);
      registeredEvent[eventName] = [];
    }
    return registeredEvent[eventName].push(callback);
  },
  unregisterSVGEvent: function(eventName, callback) {
    var callbackIndex;
  ...
  setInitialPathEditable: function(isEditable) {
    var initialPathEditable;
